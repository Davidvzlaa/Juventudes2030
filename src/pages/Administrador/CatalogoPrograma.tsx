...
import { 
  Save, Plus, Trash2, Mail, Phone, MapPin, 
  Clock, Loader2, UploadCloud, Edit2, X, Eye, EyeOff, 
  ImageIcon, GripVertical, Check, Building2, Settings, FolderOpen
} from "lucide-react";
import { toast } from "sonner";
import {
  DndContext,
...
interface SortableBannerCardProps {
  banner: HeroBanner;
  toggleBannerActivo: (banner: HeroBanner) => Promise<void>;
  iniciarEdicionBanner: (banner: HeroBanner) => void;
  setDialogoConfBanner: React.Dispatch<React.SetStateAction<{ isOpen: boolean; idBanner: string | null }>>;
  isReordering: boolean;
}
...
const MAX_CHARS = { title: 50, description: 150 };
const OPCIONES_ORDEN = Array.from({ length: 10 }, (_, index) => index + 1);
  ...
  return (
    ...
    >
      <div className="aspect-video w-full relative pointer-events-none">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-4">
          <span className="text-[10px] font-bold text-white bg-black/50 w-fit px-2 py-0.5 rounded-full mb-1">Orden: {banner.order_index}</span>
          <h4 className="text-white font-bold text-sm truncate">{(banner.title || "Sin título").replace(/\n/g, ' ')}</h4>
        </div>
      </div>
      
      {!isReordering && (
        <div className="absolute top-2 right-2 flex items-center gap-1 bg-white/90 backdrop-blur p-1 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-auto">
          <button type="button" onClick={() => toggleBannerActivo(banner)} className={`p-1.5 rounded-lg ${banner.is_active ? 'text-green-600 hover:bg-green-50' : 'text-gray-500 hover:bg-gray-100'}`}>
...
export default function CatalogoPrograma() {
  // Estado para la navegación tipo "Carpetillas" (Pestañas)
  const [tabActiva, setTabActiva] = useState<'general' | 'banners' | 'instituciones'>('general');
  ...
  const [editandoInstitucionId, setEditandoInstitucionId] = useState<number | null>(null);
  const [guardandoInstitucion, setGuardandoInstitucion] = useState(false);
  const [dialogoConfInst, setDialogoConfInst] = useState<{ isOpen: boolean; idInst: number | null }>({ isOpen: false, idInst: null });
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      if (dataBanners) {
        const bannersNormalizados = dataBanners as HeroBanner[];
        setBanners(bannersNormalizados);
        const ocupados = bannersNormalizados.map(b => b.order_index).filter((o): o is number => typeof o === 'number');
        setOrdenesOcupados(ocupados);
        
        if (!editandoBannerId) {
          let siguienteOrden = 1;
          while (ocupados.includes(siguienteOrden) && siguienteOrden <= 10) { siguienteOrden++; }
          setFormBanner(prev => ({ ...prev, order_index: siguienteOrden <= 10 ? siguienteOrden : 1 }));
        }
      }

      const { data: dataInst, error: errInst } = await supabase.from("instituciones").select("*").order("nombre", { ascending: true });
      if (!errInst && dataInst) {
  ...
  // INSTITUCIONES
  // ==========================================
  const guardarInstitucion = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!nombreInstitucion.trim()) {
      notifyWithSound("El nombre de la institución es obligatorio", "warning");
      return;
    }

    setGuardandoInstitucion(true);
    try {
      if (editandoInstitucionId) {
        const { error } = await supabase.from("instituciones").update({ nombre: nombreInstitucion.trim() }).eq("id", editandoInstitucionId);
        if (error) throw error;
        notifyWithSound("Institución actualizada correctamente", "success");
      } else {
        const { error } = await supabase.from("instituciones").insert([{ nombre: nombreInstitucion.trim(), activo: true }]);
        if (error) throw error;
        notifyWithSound("Institución registrada con éxito", "success");
      }

      setNombreInstitucion("");
      setEditandoInstitucionId(null);
      cargarDatos();
    } catch (error: any) {
      notifyWithSound(`Error: ${error.message || "No se pudo guardar la institución"}`, "error");
    } finally {
      setGuardandoInstitucion(false);
    }
  };

  const iniciarEdicionInstitucion = (inst: Institucion) => {
    setEditandoInstitucionId(inst.id);
    setNombreInstitucion(inst.nombre);
    document.getElementById('form-institucion')?.focus();
  };

  const cancelarEdicionInstitucion = () => {
    setEditandoInstitucionId(null);
    setNombreInstitucion("");
  };

  const ejecutarEliminarInstitucion = async () => {
    const id = dialogoConfInst.idInst;
    if (!id) return;
    setIsProcessingAction(true);
    try {
      ...
      notifyWithSound("Institución eliminada correctamente", "success");
      setDialogoConfInst({ isOpen: false, idInst: null });
      if (editandoInstitucionId === id) cancelarEdicionInstitucion();
    } catch (error: any) {
      notifyWithSound("Error al eliminar la institución (puede estar vinculada)", "error");
    } finally {
      setIsProcessingAction(false);
    }
  };
  ...
  const guardarNuevoOrden = async () => {
    ...
    try {
      for (let index = 0; index < banners.length; index++) {
        const banner = banners[index];
        const { error } = await supabase.from("hero_banners").update({ order_index: index + 1 }).eq("id", banner.id);
        if (error) throw error;
      }
      notifyWithSound("Orden guardado exitosamente", "success");
      setIsReordering(false);
  ...
  const guardarBanner = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (archivoBanner) {
        const nombreAnterior = obtenerNombreArchivoStorage(formBanner.image_url);
        if (nombreAnterior) {
          await supabase.storage.from('imagenes').remove([nombreAnterior]).catch(() => {});
        }
        const fileExt = archivoBanner.name.split('.').pop()?.toLowerCase() || 'jpg';
        const fileName = `banner_${Date.now()}.${fileExt}`;
        const { error: uploadError } = await supabase.storage.from('imagenes').upload(fileName, archivoBanner);
        if (uploadError) throw uploadError;
        finalImageUrl = supabase.storage.from('imagenes').getPublicUrl(fileName).data.publicUrl;
  ...
  const cancelarEdicionBanner = () => {
    setEditandoBannerId(null);
    let siguienteOrden = 1;
    const todosOcupados = banners.map(b => b.order_index).filter((o): o is number => typeof o === 'number');
    while (todosOcupados.includes(siguienteOrden) && siguienteOrden <= 10) { siguienteOrden++; }
    setFormBanner({ ...bannerVacio, order_index: siguienteOrden <= 10 ? siguienteOrden : 1 });
    setArchivoBanner(null);
    setPreviewBanner("");
    setUsaEnlacePersonalizado(false);
    setOrdenesOcupados(todosOcupados);
  ...
  const ejecutarEliminarBanner = async () => {
    ...
    try {
      const bannerToDelete = banners.find(b => b.id === dialogoConfBanner.idBanner);
      if (bannerToDelete) {
        const nombreAnterior = obtenerNombreArchivoStorage(bannerToDelete.image_url);
        if (nombreAnterior) await supabase.storage.from('imagenes').remove([nombreAnterior]).catch(() => {});
      }
      await supabase.from("hero_banners").delete().eq("id", dialogoConfBanner.idBanner);
      setBanners(banners.filter(b => b.id !== dialogoConfBanner.idBanner));
      notifyWithSound("Banner eliminado", "success");
  ...
  const processFile = (file: File, tipoId: string) => { 
    ...
    if (previewsLogo[tipoId]) {
      URL.revokeObjectURL(previewsLogo[tipoId]!);
    }
  ...
  const guardarSistema = async (e: React.FormEvent) => { 
    ...
    try { 
      ...
      const uploadPromises = Object.entries(archivosLogo).map(async ([tipoId, file]) => { 
        const urlVieja = sistema.logotipos[tipoId]; 
        const nombreAnterior = obtenerNombreArchivoStorage(urlVieja);
        if (nombreAnterior) await supabase.storage.from('imagenes').remove([nombreAnterior]).catch(() => {}); 
        const fileName = `logo_${tipoId}_${Date.now()}.${file.name.split('.').pop()}`; 
        await supabase.storage.from('imagenes').upload(fileName, file); 
        logotiposFinales[tipoId] = supabase.storage.from('imagenes').getPublicUrl(fileName).data.publicUrl; 
  ...
  const guardarRed = async () => { 
    if (!nuevaRed.url || !sistema.id) { notifyWithSound("Falta la URL o guardar el sistema primero", "warning"); return; }
    try {
      if (editandoRedId) {
        await supabase.from("redes_sociales").update({ nombre: nuevaRed.nombre, url: nuevaRed.url }).eq("id", editandoRedId);
        notifyWithSound("Red social actualizada con éxito", "success");
  ...
  return (
    <div className="w-full pb-20">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-black text-[#061A2D]">Configuración del Programa</h1>
        <p className="text-gray-500 text-sm">Gestiona la información pública, logotipos, banners y catálogos vinculados.</p>
      </div>

      {/* ================= PESTAÑAS / CARPETILLAS ================= */}
      <div className="flex overflow-x-auto gap-2 border-b border-gray-200 mb-8 custom-scrollbar pt-2">
        <button 
          onClick={() => setTabActiva('general')}
          className={`flex items-center gap-2 px-6 py-3 rounded-t-2xl font-bold text-sm transition-all border-b-4 ${tabActiva === 'general' ? 'border-[#00689D] text-[#00689D] bg-[#00689D]/5' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
        >
          <Settings size={18} /> Datos Generales
        </button>
        <button 
          onClick={() => setTabActiva('banners')}
          className={`flex items-center gap-2 px-6 py-3 rounded-t-2xl font-bold text-sm transition-all border-b-4 ${tabActiva === 'banners' ? 'border-[#26BDE2] text-[#26BDE2] bg-[#26BDE2]/5' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
        >
          <ImageIcon size={18} /> Banners de Inicio
        </button>
        <button 
          onClick={() => setTabActiva('instituciones')}
          className={`flex items-center gap-2 px-6 py-3 rounded-t-2xl font-bold text-sm transition-all border-b-4 ${tabActiva === 'instituciones' ? 'border-purple-600 text-purple-600 bg-purple-50' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
        >
          <FolderOpen size={18} /> Instituciones
        </button>
      </div>

      {/* ================= VISTA: DATOS GENERALES ================= */}
      {tabActiva === 'general' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 transition-all">
          <div className="lg:col-span-2 space-y-6">
            <form onSubmit={guardarSistema} className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm">
              <h2 className="text-lg font-bold text-gray-800 mb-6 border-b pb-4">Identidad Gráfica</h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col h-full">
              <h2 className="text-lg font-bold text-gray-800 mb-6 border-b pb-4">Redes Sociales</h2>
              <div className="overflow-y-auto mb-6 space-y-3 flex-1">
                {redes.length === 0 ? (
                  <div className="text-center text-sm text-gray-400 py-6 border-2 border-dashed border-gray-100 rounded-xl">No hay redes configuradas.</div>
                ) : (
                  redes.map(red => (
                    <div key={red.id} className={`flex items-center justify-between p-3 border rounded-xl group ${editandoRedId === red.id ? 'bg-blue-50 border-blue-200' : 'bg-gray-50'}`}>
                      <div className="flex flex-col overflow-hidden"><span className="text-xs font-black text-[#00689D]">{red.nombre}</span><span className="text-xs text-gray-500 truncate">{red.url}</span></div>
              ...
              <div className="p-4 rounded-2xl border bg-gray-50 mt-auto">
                <h3 className="text-xs font-bold text-gray-600 mb-3">{editandoRedId ? "Editando Red" : "Agregar Nueva"}</h3>
                <div className="space-y-3">
                  <select value={nuevaRed.nombre} onChange={(e) => setNuevaRed({...nuevaRed, nombre: e.target.value})} className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm"><option>Facebook</option><option>Instagram</option><option>LinkedIn</option></select>
                  <input type="url" placeholder="URL" value={nuevaRed.url} onChange={(e) => setNuevaRed({...nuevaRed, url: e.target.value})} className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm" />
                  <div className="flex gap-2">
                    {editandoRedId && <button type="button" onClick={() => { setEditandoRedId(null); setNuevaRed({nombre: 'Facebook', url: ''}); }} className="px-3 py-2 bg-white border rounded-xl text-xs font-bold">Cancelar</button>}
                    <button type="button" onClick={guardarRed} className="flex-1 bg-[#26BDE2] text-white py-2 rounded-xl text-sm font-bold">{editandoRedId ? "Actualizar" : "Agregar"}</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* ================= VISTA: BANNERS ================= */}
      {tabActiva === 'banners' && (
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm transition-all">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 pb-4 border-b border-gray-100">
            <div>
              <h2 className="text-xl font-bold text-gray-800 mb-1">Banners del Carrusel</h2>
              <p className="text-gray-500 text-sm">Controla las imágenes y textos principales que se muestran al inicio.</p>
            </div>
            ...
            {banners.length > 1 && (
              <div className="mt-4 sm:mt-0">
                {isReordering ? (
                  <div className="flex gap-2">
                    <button onClick={() => { setIsReordering(false); cargarDatos(); }} className="px-4 py-2 border border-gray-300 rounded-xl text-sm font-bold text-gray-700 bg-white hover:bg-gray-50">Cancelar</button>
                    <button onClick={guardarNuevoOrden} disabled={isSavingOrder} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-bold hover:bg-green-700 disabled:opacity-50">
                      {isSavingOrder ? <Loader2 size={16} className="animate-spin" /> : <Check size={16} />} Guardar Orden
                    </button>
                  </div>
                ) : (
                  <button onClick={() => { setIsReordering(true); cancelarEdicionBanner(); }} className="flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 border border-gray-200 rounded-xl text-sm font-bold hover:bg-gray-200 transition-colors">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-4">
              {banners.length === 0 ? (
                <div className="text-center text-sm text-gray-400 py-16 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">Aún no has agregado banners al carrusel.</div>
              ) : (
                <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                  <SortableContext items={banners.map(b => b.id || '')} strategy={rectSortingStrategy}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {banners.map(banner => (
                        <SortableBannerCard key={banner.id} banner={banner} toggleBannerActivo={toggleBannerActivo} iniciarEdicionBanner={iniciarEdicionBanner} setDialogoConfBanner={setDialogoConfBanner} isReordering={isReordering} />
                      ))}
                    </div>
                  </SortableContext>
                </DndContext>
            ...
            <div className="lg:col-span-1 relative">
              {isReordering && (
                <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-10 flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300">
                  <GripVertical size={32} className="text-gray-400 mb-2 opacity-50" />
                  <span className="text-gray-600 font-bold text-sm bg-white px-4 py-2 rounded-full shadow-sm">Guarda el orden para editar</span>
                </div>
              )}
              ...
              <form onSubmit={guardarBanner} className={`p-5 rounded-2xl border transition-colors ${editandoBannerId ? 'bg-blue-50/50 border-blue-200' : 'bg-gray-50 border-gray-200'}`}>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Imagen de fondo</label>
                    <div onClick={() => bannerFileInputRef.current?.click()} className="w-full aspect-video border-2 border-dashed border-gray-300 rounded-xl overflow-hidden cursor-pointer hover:border-[#00689D] relative flex flex-col items-center justify-center bg-white">
                      <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" ref={bannerFileInputRef} onChange={handleBannerFileChange} />
                      {previewBanner ? <img src={previewBanner} className="w-full h-full object-cover" /> : <><ImageIcon className="w-8 h-8 text-gray-400 mb-2" /><span className="text-xs text-gray-500">Clic para subir imagen</span></>}
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-end mb-1">
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Título principal</label>
                      <span className={`text-[10px] font-bold ${formBanner.title.length >= MAX_CHARS.title ? 'text-red-500' : 'text-gray-400'}`}>{formBanner.title.length}/{MAX_CHARS.title}</span>
                    </div>
                    <textarea required maxLength={MAX_CHARS.title} rows={2} placeholder="Ej. Juventudes&#10;que transforman." value={formBanner.title} onChange={(e) => setFormBanner({...formBanner, title: e.target.value})} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-[#00689D]/50 resize-none" />
                  </div>

                  <div>
                    <div className="flex justify-between items-end mb-1">
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Descripción (Opcional)</label>
                      <span className={`text-[10px] font-bold ${formBanner.description.length >= MAX_CHARS.description ? 'text-red-500' : 'text-gray-400'}`}>{formBanner.description.length}/{MAX_CHARS.description}</span>
                    </div>
                    <textarea maxLength={MAX_CHARS.description} rows={3} placeholder="Breve texto descriptivo..." value={formBanner.description} onChange={(e) => setFormBanner({...formBanner, description: e.target.value})} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-[#00689D]/50 resize-none" />
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    <div>
                      ...
                      {usaEnlacePersonalizado ? (
                        <input type="url" required placeholder="Ej. https://..." value={formBanner.cta_link} onChange={(e) => setFormBanner({...formBanner, cta_link: e.target.value})} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00689D]/50" />
                      ) : (
                        <select required value={formBanner.cta_link} onChange={(e) => setFormBanner({...formBanner, cta_link: e.target.value})} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#00689D]/50">
                          <option value="" disabled>Selecciona una página...</option>
                          {PAGINAS_DISPONIBLES.map(pagina => (<option key={pagina.value} value={pagina.value}>{pagina.label}</option>))}
                  ...
                  <div className="grid grid-cols-2 gap-3 items-center pt-2">
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Orden de aparición</label>
                      <select required value={formBanner.order_index} onChange={(e) => setFormBanner({...formBanner, order_index: parseInt(e.target.value)})} className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-[#00689D]/50">
                        {OPCIONES_ORDEN.map(num => (<option key={num} value={num} disabled={ordenesOcupados.includes(num) && formBanner.order_index !== num}>{num} {ordenesOcupados.includes(num) && formBanner.order_index !== num ? "(Ocupado)" : ""}</option>))}
                      </select>
                    </div>
                    <div className="flex flex-col items-end justify-center pt-3">
                      <label className="flex items-center gap-2 cursor-pointer">
      ...
      {/* ================= VISTA: INSTITUCIONES ("Carpetillas") ================= */}
      {tabActiva === 'instituciones' && (
        <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-100 shadow-sm transition-all">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 pb-4 border-b border-gray-100">
            <div>
              <h2 className="text-xl font-bold text-gray-800 mb-1 flex items-center gap-2">
                <Building2 className="text-blue-600" /> Catálogo de Instituciones
              </h2>
              <p className="text-gray-500 text-sm">Administra las instituciones u organismos vinculados al proyecto.</p>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-1 sticky top-4">
              <form onSubmit={guardarInstitucion} className="p-5 rounded-2xl border border-purple-100 bg-blue-50/30 space-y-4">
                <div>
                  <h3 className="text-sm font-bold text-gray-800 mb-1">
                    {editandoInstitucionId ? "Editar Institución" : "Registrar Nueva"}
                  </h3>
                  <p className="text-[10px] text-gray-500 mb-3">Escribe el nombre oficial para utilizarlo en otras áreas del sistema.</p>
                </div>
                <input 
                  id="form-institucion"
                  type="text" 
                  ...
                  className="w-full bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50" 
                />
                <div className="flex gap-2 pt-2">
                  {editandoInstitucionId && (
                    <button type="button" onClick={cancelarEdicionInstitucion} className="px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm font-bold hover:bg-gray-100">
                      Cancelar
                    </button>
                  )}
                  <button type="submit" disabled={guardandoInstitucion} className="flex-1 bg-blue-600 text-white py-2.5 rounded-xl text-sm font-bold flex justify-center items-center gap-2 hover:bg-purple-700 disabled:opacity-50">
                    {guardandoInstitucion ? <Loader2 className="w-4 h-4 animate-spin" /> : (editandoInstitucionId ? <Save className="w-4 h-4"/> : <Plus className="w-4 h-4"/>)}
                    {editandoInstitucionId ? "Actualizar" : "Guardar"}
                  </button>
                </div>
              </form>
            </div>

            {/* Grid de Instituciones ("Carpetillas / Tarjetas") */}
            <div className="lg:col-span-2">
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {instituciones.length === 0 ? (
                  <div className="col-span-full text-center text-sm text-gray-400 py-16 border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">
                    Aún no hay instituciones registradas en el catálogo.
                  </div>
                ) : (
                  instituciones.map(inst => (
                    <div key={inst.id} className="relative group bg-white border border-gray-200 rounded-2xl p-4 hover:border-blue-300 hover:shadow-md transition-all flex flex-col items-center justify-center text-center gap-3 aspect-square cursor-default">
                      
                      
                      
                      <span className="text-xs font-bold text-gray-800 line-clamp-3 leading-tight">{inst.nombre}</span>

                      {/* Botones de acción flotantes */}
                      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white/90 p-1 rounded-lg backdrop-blur shadow-sm">
                        <button type="button" onClick={() => iniciarEdicionInstitucion(inst)} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-md"><Edit2 className="w-3.5 h-3.5" /></button>
                        <button type="button" onClick={() => setDialogoConfInst({ isOpen: true, idInst: inst.id })} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-md"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
      ...
      {dialogoConfInst.isOpen && (
        <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 text-center">
            <h3 className="text-xl font-bold mb-2">Eliminar Institución</h3>
            <p className="text-sm text-gray-600 mb-6">¿Estás seguro de eliminar esta institución?</p>
            <div className="flex gap-3 w-full">
              <button type="button" onClick={() => setDialogoConfInst({ isOpen: false, idInst: null })} className="flex-1 px-4 py-2.5 border rounded-xl text-sm font-bold text-gray-700">Cancelar</button>
              <button type="button" onClick={ejecutarEliminarInstitucion} className="flex-1 px-4 py-2.5 bg-red-600 rounded-xl text-sm font-bold text-white">Sí, eliminar</button>
            </div>
          </div>
        </div>
      )}
