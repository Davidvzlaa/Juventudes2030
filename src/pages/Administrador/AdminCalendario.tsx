...
export default function AdminAgenda() {
  ...
  const fetchDatosBase = async () => {
    ...
    try {
      ...
      // Admin solo ve sus propios borradores
      const actividadesPermitidas = (acts as any[]).filter(a => a.estado !== 'Borrador' || a.creado_por_usuario_id === usuarioDatos.id);
      setActividades(actividadesPermitidas || []);
  ...
  const obtenerEstilosUI = (act: Actividad) => {
    ...
    } else if (esCreador && act.es_externa) {
      return {
        mes: 'bg-purple-50 text-purple-700 border border-purple-200 hover:bg-purple-600 hover:text-white cursor-pointer hover:scale-[1.02] transform transition-transform',
        semana: 'bg-white border-purple-600',
        diaMain: 'border-purple-200 bg-white', diaHora: 'bg-purple-50 text-purple-700 border-purple-200', drawerHeader: 'bg-purple-600'
      };
    } else if (esCreador) {
      return {
        mes: 'bg-blue-50 text-[#00689D] border border-blue-200 hover:bg-[#00689D] hover:text-white cursor-pointer hover:scale-[1.02] transform transition-transform',
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    // Validación Dinámica de Instituciones vs Domicilio
    if (formData.es_externa) {
      if (isNuevaInstitucion && !formData.nueva_institucion.trim()) {
        return notifyWithSound('Escribe el nombre de la nueva institución.', 'warning');
      } else if (!isNuevaInstitucion && !formData.institucion_id) {
        return notifyWithSound('Selecciona la institución del catálogo.', 'warning');
      }
    } else {
      if (!formData.calle.trim() || !formData.colonia.trim()) {
        return notifyWithSound("La calle y colonia son obligatorias para actividades propias.", 'warning');
      }
    }
    ...
    try {
      // 1. Procesar nueva institución si aplica
      let finalInstId = formData.institucion_id;
      if (formData.es_externa && isNuevaInstitucion && formData.nueva_institucion) {
        const nombreBuscado = formData.nueva_institucion.trim().toLowerCase();
        const existe = institucionesDB.find(i => i.nombre.toLowerCase() === nombreBuscado);
        
        if (existe) {
          finalInstId = existe.id;
        } else {
          const { data: nuevaInst, error: errInst } = await supabase.from('instituciones').insert([{ nombre: formData.nueva_institucion.trim(), activo: true }]).select().single();
      ...
      await guardarActividadCalendario({
        ...
        odsIds: odsSeleccionados,
        es_externa: formData.es_externa,
        institucion_id: formData.es_externa ? finalInstId : null
      });
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4 mt-3 text-sm text-gray-600 font-medium">
                  ...
                  <span className="flex items-center gap-1.5">
                    {act.es_externa ? <Building2 size={16} className="text-purple-400"/> : <Info size={16} className="text-gray-400"/>}
                    {act.es_externa && act.instituciones?.nombre ? act.instituciones.nombre : (act.municipios?.nombre || 'General')}
                  </span>
                </div>
              </div>
              <div className="flex items-center justify-end">
                  ...
                  {filtradas.map((a) => {
                    ...
                    return (
                      <tr key={a.id} className="hover:bg-blue-50/50 transition-colors">
                        ...
                        <td className="p-4 text-center">
                          <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider inline-block ${esCrea ? (esInst ? 'bg-purple-100 text-purple-700' : 'bg-[#00689D]/10 text-[#00689D]') : 'bg-slate-100 text-slate-600'}`}>
                            {esCrea ? (esInst ? 'Admin (Inst.)' : 'Admin (Tú)') : 'Embajador'}
                          </span>
                        </td>
                        <td className="p-4">
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold border ${a.estado === 'Borrador' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : a.estado === 'Cancelada' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-green-50 text-green-700 border-green-200'}`}>
    ...
    <div className="max-w-7xl mx-auto pb-12 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-gray-500 mt-2 text-base max-w-xl">
            Supervisa las actividades de todos los embajadores o crea eventos de alcance global e institucional.
          </p>
        </div>
        <div className=" flex justify-end">
         <div className="relative w-full sm:w-72">
      ...
      {mainTab === 'calendario' ? (
        <>
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6 px-2 text-xs font-medium text-gray-600">
            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-[#00689D]"></div> Creadas por Mí (Admin)</span>
            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-purple-600"></div> Instituciones</span>
            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-slate-400"></div> Creadas por Embajadores</span>
            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full border-2 border-dashed border-amber-400 bg-amber-100"></div> Mis Borradores</span>
            <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full border border-red-300 bg-red-100 text-red-500 flex items-center justify-center font-bold text-[8px]">X</div> Cancelada</span>
          </div>
              ...
              <div className="flex items-start gap-4 pt-6 border-t border-gray-100">
                ...
                <div>
                  ...
                  <p className="font-bold text-gray-900 text-lg">
                    ...
                    {eventoSeleccionado.creado_por_usuario_id === usuarioDatos?.id && !eventoSeleccionado.es_externa && <span className={`ml-2 font-black text-sm ${eventoSeleccionado.estado === 'Cancelada' ? 'text-red-600' : 'text-[#00689D]'}`}>(ADMIN)</span>}
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8">
              <form id="form-actividad-admin" onSubmit={handleSubmitForm} className="space-y-6">
                  ...
                  <div className="bg-purple-50/50 p-4 border border-purple-100 rounded-xl mb-4">
                    ...
                    {formData.es_externa && (
                      <div className="mt-4 ml-6 p-4 bg-white border border-purple-100 rounded-lg shadow-sm">
                        ...
                        {!isNuevaInstitucion ? (
                          <select 
                            required name="institucion_id" value={formData.institucion_id || ''} 
                            onChange={(e) => setFormData({...formData, institucion_id: Number(e.target.value)})} 
                            className="w-full border border-gray-300 rounded-lg p-2 text-sm bg-white outline-none focus:border-purple-500"
                          >
                          ...
                          <input 
                            required type="text" name="nueva_institucion" value={formData.nueva_institucion || ''} 
                            onChange={(e) => setFormData({...formData, nueva_institucion: e.target.value})} placeholder="Escribe el nombre de la institución..."
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1">Dirección Detallada {!formData.es_externa && <span className="text-red-500">*</span>}</label>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <input type="text" required={!formData.es_externa} className="w-full p-2.5 border border-gray-300 rounded-lg outline-none focus:border-[#00689D] text-sm" value={formData.calle} onChange={(e) => setFormData({...formData, calle: e.target.value})} placeholder="Calle y número" />
                      <input type="text" required={!formData.es_externa} className="w-full p-2.5 border border-gray-300 rounded-lg outline-none focus:border-[#00689D] text-sm" value={formData.colonia} onChange={(e) => setFormData({...formData, colonia: e.target.value})} placeholder="Colonia" />
                      <input type="text" className="w-full p-2.5 border border-gray-300 rounded-lg outline-none focus:border-[#00689D] text-sm" value={formData.direccion} onChange={(e) => setFormData({...formData, direccion: e.target.value})} placeholder="Referencias" />
                    </div>
                  </div>
                </div>
