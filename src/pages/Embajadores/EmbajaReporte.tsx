...
const ESTADOS_BLOQUEADOS: EstadoReporte[] = [
  'Enviado',
  'Aprobado'
];
...
const notifyWithSound = (
  ...
  try {
    ...
    // El audio no debe impedir la operación principal.
  }
...
export default function EmbajaReporte() {
  ...
  const [actividadEnEdicion, setActividadEnEdicion] =
  ...
  const [accionesDB, setAccionesDB] =
    useState<Catalogo[]>([]);

  const [odsDB, setOdsDB] =
    useState<Catalogo[]>([]);

  const [municipiosDB, setMunicipiosDB] =
    useState<Catalogo[]>([]);

  const [sectoresDB, setSectoresDB] =
    useState<Catalogo[]>([]);

  const [areasSostenibilidadDB, setAreasSostenibilidadDB] =
    useState<Catalogo[]>([]);

  const [embajadoresLocal, setEmbajadoresLocal] =
    useState<EmbajadorLocal[]>([]);
  ...
  const [evidenciaToDelete, setEvidenciaToDelete] =
  ...
  const reporteBloqueado =
    reporteSeleccionado
      ? ESTADOS_BLOQUEADOS.includes(reporteSeleccionado.estado)
      : false;

  const esPropietario =
    actividadEnEdicion?.es_propia !== false;
    ...
    try {
      ...
      const [
        ...
        supabase
          .from('ods')
          .select(
            'id, numero, nombre, categoria_sostenibilidad'
          )
          .eq('activo', true)
          .order('numero'),
        ...
        supabase
          .from('sectores_poblacion')
          .select('id, nombre')
          .eq('activo', true)
          .order('id'),

        supabase
          .from('areas_sostenibilidad')
          .select('id, nombre')
          .eq('activo', true)
          .order('id'),
        ...
        supabase
          .from('reportes')
          .select(
            'id, mes, anio, estado, periodo_inicio, periodo_fin'
          )
      ...
      if (resCat.data) {
        setCategoriasDB([
          ...
          {
            id: 99,
            nombre: 'Total Beneficiarios'
          }
        ]);
      }

      if (resAcc.data) {
        setAccionesDB(resAcc.data);
      }

      if (resOds.data) {
        setOdsDB(resOds.data);
      }

      if (resMun.data) {
        setMunicipiosDB(resMun.data);
      }

      if (resSec.data) {
        setSectoresDB(resSec.data);
      }

      if (resAreas.data) {
        setAreasSostenibilidadDB(resAreas.data);
      }
      ...
      console.error(
        'Error cargando datos del reporte:',
        error
      );

      notifyWithSound(
        `No se pudieron cargar tus reportes: ${obtenerMensajeError(error)}`,
        'error'
      );
  ...
  const seleccionarReporte = async (
    reporte: Reporte
  ) => {
    setReporteSeleccionado(reporte);
    setActividades([]);
    ...
    try {
      ...
      actividadesUnidas?.forEach(
        (item: any) => {
          const act = Array.isArray(
            item.actividades
          )
            ? item.actividades[0]
            : item.actividades;
          ...
          if (
            act.fecha_evento < fechaInicio ||
            act.fecha_evento > fechaFin
          ) {
            return;
          }
      ...
      arrayActividades.forEach(
        (act: any) => {
          const misEvidencias =
            act.evidencias?.filter(
              (ev: any) =>
                ev.usuario_id === currentUserId
            ) || [];

          misEvidencias.forEach(
            (ev: any) => {
              if (ev.url_archivo) {
                allEvidencePaths.push(
                  ev.url_archivo
                );
              }
            }
      ...
      if (allEvidencePaths.length > 0) {
        const pathsPropios = [...new Set(allEvidencePaths)]
          .filter((path) => path.startsWith(`${currentUserId}/`));
        if (pathsPropios.length > 0) {
          const {
            data: signedData,
            error: signedErr
      ...
      const actividadesCompletas:
        ActividadFormulario[] =
        arrayActividades.map(
          (act: any) => {

            const beneficiariosObj:
              Record<
                number,
                {
                  hombres: string;
            ...
            act.actividad_beneficiarios?.forEach(
              (b: any) => {
                beneficiariosObj[b.categoria_id] = {
                  hombres:
                    b.hombres?.toString() || '0',
                  mujeres:
                    b.mujeres?.toString() || '0',
                  total:
                    b.total?.toString() || '0'
                };
              }
            ...
            act.actividad_sectores?.forEach(
              (s: any) => {
                sectoresObj[s.sector_id] = {
                  hombres:
                    s.hombres?.toString() || '0',
                  mujeres:
                    s.mujeres?.toString() || '0',
                  total:
                    s.total?.toString() || '0'
                };
              }
            ...
            const odsSeleccionados: number[] = [];

            if (act.actividad_ods) {
              const principal =
                act.actividad_ods.find(
                  (o: any) =>
                    o.es_principal
                );

              const secundarios =
                act.actividad_ods.filter(
                  (o: any) =>
                    !o.es_principal
                );

              if (principal) {
                odsSeleccionados.push(
                  principal.ods_id
                );
              }
            ...
            let tipoActividadNombre = '';

            if (
              act.actividad_acciones &&
              act.actividad_acciones.length > 0
            ) {
              const accionRel =
                act.actividad_acciones[0];

              const accionCat =
                accionesDB.find(
                  (a) =>
                    a.id ===
                    accionRel.tipo_accion_id
                );
            ...
            const evidenciasConUrlTemporal:
              Evidencia[] =
              misEvidencias.map(
                (ev: any) => ({
                  id: ev.id,
            ...
            return {
              ...
              es_externa:
                Boolean(act.es_externa),

              organizador_externo:
                act.organizador_externo || ''
            } as ActividadFormulario;
          }
        );
      ...
      notifyWithSound(
        `Error al cargar las actividades: ${obtenerMensajeError(error)}`,
        'error'
      );
    }
  };
  ...
  // ==========================================================
  // CARGAR EMBAJADORES DEL MUNICIPIO
  // ==========================================================
  ...
  useEffect(() => {
    const cargarEmbajadores = async () => {
      ...
      setEmbajadoresLocal(
        filtrados as unknown as EmbajadorLocal[]
      );
    };
    ...
    actividadEnEdicion?.es_colaborativa,
    actividadEnEdicion?.domicilio?.municipio,
    currentUserId
  ]);
  ...
  useEffect(() => {
    return () => {
      if (
        actividadEnEdicion?.evidencias
      ) {
        actividadEnEdicion.evidencias.forEach(
          (ev) => {
            if (
              ev.url &&
  ...
  const toggleOds = (
    ...
    setActividadEnEdicion(
      (prev) => {
        ...
        const arr =
          prev.ods_seleccionados ||
          [];

        if (arr.includes(odsId)) {
          return {
            ...prev,
            ods_seleccionados:
              arr.filter(
        ...
        if (
          ...
          notifyWithSound(
            `Solo puedes seleccionar un máximo de ${MAX_ODS} ODS.`,
            'warning'
          );
    ...
    setActividadEnEdicion(
      (prev) => {
        ...
        if (
          ...
          notifyWithSound(
            `Límite alcanzado: solo tienes ${maxPermitido} ${campo} disponibles según tu registro de categorías.`,
            'warning'
          );
    ...
    setActividadEnEdicion(
      (prev) => {
        ...
        if (
          ...
          notifyWithSound(
            `Máximo ${MAX_EVIDENCIAS} evidencias permitidas por actividad.`,
            'warning'
          );
  ...
  const confirmEliminarEvidencia =
    async () => {
      if (
      ...
      const evEliminar =
        actividadEnEdicion.evidencias.find(
          (ev) =>
            ev.id ===
            idEliminar
        );
      ...
      // ----------------------------------------------
      // Evidencia nueva aún no subida
      // ----------------------------------------------

      if (evEliminar.file) {
        if (
          evEliminar.url.startsWith(
            'blob:'
          )
        ) {
          URL.revokeObjectURL(
      ...
      try {
        if (
          !evEliminar.path_interno
        ) {
          throw new Error(
            'No se encontró la ruta interna de la evidencia.'
          );
        }
        ...
        toast.error(
          `No se pudo eliminar la evidencia: ${obtenerMensajeError(error)}`,
          {
            id: toastId
          }
        );
  ...
  // ==========================================================
  // SUBIR SOLAMENTE EVIDENCIAS DE ACTIVIDAD COMPARTIDA
  // ==========================================================

  const guardarEvidenciasActividadCompartida =
    async () => {
      if (
        !actividadEnEdicion ||
        !currentUserId
      ...
      if (
        !actividadId ||
        String(actividadId).startsWith(
          'act-'
        )
      ) {
        throw new Error(
          'La actividad compartida no tiene un ID válido.'
        );
      }

      const archivosNuevos =
        actividadEnEdicion.evidencias.filter(
          (ev) => Boolean(ev.file)
        );
      ...
      for (
        ...
        const fileName =
          `${actividadId}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}.${extension}`;
  ...
  const guardarEdicionActividad =
    async (
      ...
      if (reporteBloqueado) {
        notifyWithSound(
          'Este reporte ya fue enviado o aprobado y no admite modificaciones.',
          'warning'
        );
        return;
      }
      ...
      if (!esPropietario) {
        ...
        try {
          ...
          notifyWithSound(
            'Tus evidencias fueron guardadas correctamente.',
            'success'
          );

          setActividadEnEdicion(null);

          await seleccionarReporte(
            reporteSeleccionado
          );
          ...
          notifyWithSound(
            `No se pudieron guardar tus evidencias: ${obtenerMensajeError(error)}`,
            'error'
          );
      ...
      if (
        actividadEnEdicion.es_colaborativa &&
        !actividadEnEdicion.colaborador_id
      ) {
        notifyWithSound(
          'Selecciona el embajador colaborador de la lista.',
          'warning'
        );
      ...
      if (
        !actividadEnEdicion.nombre.trim()
      ) {
        notifyWithSound(
      ...
      if (
        !actividadEnEdicion.tipo_actividad
      ) {
        notifyWithSound(
      ...
      if (
        !actividadEnEdicion.fecha_evento
      ) {
        notifyWithSound(
          'Selecciona la fecha de la actividad.',
          'warning'
        );
      ...
      if (
        !actividadEnEdicion.es_externa &&
        (
          !actividadEnEdicion.domicilio.colonia ||
          !actividadEnEdicion.domicilio.calle
        )
      ) {
        notifyWithSound(
          'La colonia y la calle son obligatorias para actividades propias.',
          'warning'
        );
      ...
      if (
        actividadEnEdicion.ods_seleccionados.length ===
        0
      ) {
        notifyWithSound(
          'Selecciona al menos un ODS.',
          'warning'
        );
      ...
      if (
        ...
        notifyWithSound(
          `Solo puedes seleccionar ${MAX_ODS} ODS como máximo.`,
          'warning'
        );
      ...
      if (
        actividadEnEdicion.es_externa &&
        !actividadEnEdicion.organizador_externo.trim()
      ) {
        notifyWithSound(
          'Indica el nombre de la institución u organizador externo.',
          'warning'
        );
      ...
      if (
        !actividadEnEdicion.es_externa &&
        !actividadEnEdicion.rango_edad.trim()
      ) {
        notifyWithSound(
          'Indica el rango de edad de los beneficiarios.',
          'warning'
        );
      ...
      try {
        ...
        const payloadBeneficiarios:
          any[] = [];

        const payloadSectores:
          any[] = [];
        ...
        if (
          ...
          Object.entries(
            actividadEnEdicion.beneficiarios ||
              {}
          ).forEach(
            ([catIdStr, val]) => {
              const catId =
          ...
          if (
            ...
            notifyWithSound(
              'La cantidad registrada en Sectores supera el total de Beneficiarios.',
              'error'
            );
          ...
          if (
            ...
            notifyWithSound(
              'Registra al menos una persona beneficiada.',
              'warning'
            );
        ...
        const agregarArea = (area: Catalogo | undefined) => {
          if (area) areasSeleccionadas.add(area.id);
        };
        ...
        actividadEnEdicion
          ...
          ?.forEach(
            (odsId, idx) => {
              ...
              if (
                odsObj?.categoria_sostenibilidad
              ) {
                const cat =
                  odsObj.categoria_sostenibilidad
                    .toLowerCase();
                ...
                if (
                  ...
                  areasSostenibilidadDB.forEach((area) => areasSeleccionadas.add(area.id));
                }
              }
            }
          );
        ...
        const payloadTransaccion = {
          ...
          actividad: {
            ...
            hora_inicio:
              actividadEnEdicion.hora_inicio ||
              null,

            hora_fin:
              actividadEnEdicion.hora_fin ||
              null,

            lugar:
              actividadEnEdicion.lugar.trim(),
            ...
            calle:
              actividadEnEdicion.domicilio?.calle?.trim() ||
              null,

            colonia:
              actividadEnEdicion.domicilio?.colonia?.trim() ||
              null,

            rango_edad_beneficiarios:
              actividadEnEdicion.es_externa
                ? null
                : actividadEnEdicion.rango_edad.trim(),

            descripcion:
              actividadEnEdicion.descripcion.trim(),
        ...
        const {
          ...
          'guardar_actividad_transaccional',
          {
            payload:
              payloadTransaccion
          }
        ...
        if (
          ...
          throw new Error(
            'La actividad se procesó pero la base de datos no devolvió su ID.'
          );
        }
        ...
        for (
          ...
          if (!ev.file.type.startsWith('image/')) {
            throw new Error(`La evidencia "${ev.file.name}" no es una imagen válida.`);
          }

          if (ev.file.size > 10 * 1024 * 1024) {
            throw new Error(`La evidencia "${ev.file.name}" supera el límite de 10 MB.`);
          }
          ...
          const fileName =
            `${savedActId}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}.${extension}`;
        ...
        notifyWithSound(
          `Error al guardar: ${obtenerMensajeError(error)}`,
          'error'
        );
  ...
  const crearNuevaActividad =
    () => {
      ...
      if (reporteBloqueado) {
        notifyWithSound(
          'Este reporte ya no permite agregar actividades.',
          'warning'
        );
      ...
      setActividadEnEdicion({
        ...
        fecha_evento:
          `${reporteSeleccionado.anio}-${mesStr}-01`,
    ...
    () => {
      ...
      if (
        ...
        return {
          listo: false,
          msg:
            'Este reporte ya fue enviado o aprobado.'
        };
      }
      ...
      if (
        ...
        return {
          listo: false,
          msg:
            'No tienes actividades registradas en este mes.'
        };
      }

      for (
        const act of actividades
      ) {

        if (
          !act.evidencias ||
          act.evidencias.length ===
            0
        ) {
          return {
            listo: false,
            msg:
              `La actividad "${act.nombre || 'sin nombre'}" requiere al menos 1 fotografía de evidencia.`
          };
        }
        ...
        const faltaInfoBasica =
          ...
          !act.hora_fin ||
          !act.lugar?.trim() ||
          !act.domicilio?.municipio ||
          !act.descripcion?.trim();

        const faltaDomicilio =
          !act.es_externa &&
          (
            !act.domicilio?.colonia?.trim() ||
            !act.domicilio?.calle?.trim()
          );
        ...
        if (
          ...
          return {
            listo: false,
            msg:
              `La actividad "${act.nombre || 'sin nombre'}" tiene campos obligatorios vacíos.`
          };
        }
        ...
        if (
          !act.ods_seleccionados ||
          act.ods_seleccionados.length ===
            0
        ) {
          return {
            listo: false,
            msg:
              `La actividad "${act.nombre}" debe tener al menos 1 ODS seleccionado.`
          };
        }
        ...
        if (
          ...
          return {
            listo: false,
            msg:
              `La actividad "${act.nombre}" tiene más de ${MAX_ODS} ODS.`
          };
        }
        ...
        if (
          ...
          if (
            !act.organizador_externo?.trim()
          ) {
            return {
              listo: false,
              msg:
                `La actividad externa "${act.nombre}" requiere el nombre de la institución que organizó.`
            };
          }
        ...
        if (
          ...
          return {
            listo: false,
            msg:
              `La actividad "${act.nombre}" requiere especificar el rango de edad.`
          };
        }
        ...
        if (
          ...
          return {
            listo: false,
            msg:
              `La actividad "${act.nombre}" debe tener al menos una persona beneficiada registrada.`
          };
        }
      }
  ...
  const handleEnviarReporte =
    async () => {
      ...
      try {
        ...
        toast.error(
          `Error al enviar el reporte: ${obtenerMensajeError(error)}`
        );
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full min-h-[500px] text-gray-500">
        <Loader2
          className="animate-spin mb-4 text-[#00689D]"
          size={40}
        />
      ...
      >
        <AlertDialogContent className="bg-white border border-gray-200 shadow-2xl rounded-2xl">
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-600">
              Estás a punto de enviar tu reporte mensual.
              Una vez enviado, no podrás modificar ni agregar
              actividades a este mes a menos que el comité lo
              regrese para correcciones.
              <br />
              <br />
              ¿Revisaste que todas tus evidencias y beneficiarios
              estén correctos?
            </AlertDialogDescription>
          ...
          <AlertDialogFooter className="mt-4">
            ...
            <AlertDialogCancel
              disabled={isSending}
              className="bg-gray-100 border-none font-bold hover:bg-gray-200 text-gray-700"
            >
              Revisar de nuevo
            </AlertDialogCancel>
        ...
        <AlertDialogContent className="bg-white border border-gray-200 shadow-2xl rounded-2xl">
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-600">
              ...
              fotografía permanentemente?
              <br />
              Esta acción eliminará el archivo almacenado y
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 shrink-0 rounded-t-2xl">
              ...
              <h3 className="font-black text-xl text-gray-800 flex items-center gap-2">
                ...
                {String(
                  ...
                  : esPropietario
                    ? 'Editar Actividad'
                    : 'Detalles de Actividad Compartida'}
              ...
              {!esPropietario && (
                <div className="mb-6 bg-blue-50 border border-blue-200 text-blue-800 p-4 rounded-xl flex gap-3 text-sm">
                  ...
                  <p>
                    ...
                    Este evento fue creado por otro embajador.
                    Puedes consultar sus detalles y subir tus
                    propias evidencias fotográficas para que
                    formen parte de tu reporte.
                    <br />
                    <br />
                    La información general de la actividad
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  {esPropietario && (
                    <div className="bg-purple-50/50 p-4 border border-purple-100 rounded-xl mb-4">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer mb-2">
                        ...
                        <span className="text-sm font-bold text-gray-800">
                          ¿Fue una actividad organizada por un tercero
                          (Escuela, Institución, Empresa)?
                        </span>
                      ...
                      {actividadEnEdicion.es_externa && (
                        <div className="mt-3 ml-6">
                          ...
                          <input
                            ...
                            name="organizador_externo"
                            value={
                              actividadEnEdicion.organizador_externo ||
                              ''
                            }
                            onChange={
                              handleChangeSimple
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:border-[#00689D] outline-none disabled:bg-gray-100 disabled:text-gray-500"
                      />
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        name="fecha_evento"
                        value={
                          actividadEnEdicion.fecha_evento?.split(
                            'T'
                          )[0] ||
                          ''
                        }
                        ...
                        className="w-full border border-gray-300 rounded-lg p-2 text-sm disabled:bg-gray-100 disabled:text-gray-500"
                      />
                  ...
                  <div className="bg-blue-50/40 p-4 rounded-xl border border-blue-100 mt-2">
                    ...
                    <label className="flex items-center gap-1 text-xs font-bold text-[#00689D] mb-2">
                      ...
                      <span className="font-normal">
                        (Máximo 4 - El primero es el Principal)
                      </span>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {odsDB.map(
                        (ods) => {
                          ...
                          const isSelected =
                            actividadEnEdicion.ods_seleccionados?.includes(
                              ods.id
                            );

                          const isPrincipal =
                            actividadEnEdicion.ods_seleccionados?.[0] ===
                            ods.id;
                              ...
                              className={`
                                ...
                                ${
                                  isSelected
                                    ...
                                    : 'bg-white text-gray-500 hover:border-blue-300'
                                }
                                ${
                                  !esPropietario
                      ...
                      <div className="bg-orange-50/50 p-4 border border-orange-100 rounded-xl mt-4">
                        ...
                        <label className="flex items-center gap-2 cursor-pointer mb-2">
                          ...
                          <span className="text-sm font-bold text-gray-800 flex items-center gap-1">
                            ...
                            ¿Asististe a esta actividad organizada
                            por otro embajador?
                        ...
                        {actividadEnEdicion.es_colaborativa && (
                          <div className="mt-3 ml-6">
                            ...
                            >
                              ...
                              {embajadoresLocal.length ===
                              0 ? (
                                <option
                                  value=""
                                  disabled
                                >
                                  No hay más embajadores
                                  en este municipio
                                </option>
                              ) : (
                                embajadoresLocal.map(
                                  (emb) => (
                                    <option
                                      key={
                                        emb.usuario_id
                        ...
                        (cat) => {
                          ...
                          return (
                            <div
                              ...
                              className={`
                                flex gap-2 items-center p-2 rounded-lg border
                                ${
                                  esFilaTotal
                                    ? 'bg-[#00689D]/5 border-[#00689D]/20 mt-4'
                            ...
                            >
                              ...
                              <span
                                className={`
                                  w-1/3 text-[10px] leading-tight
                                  ${
                                    esFilaTotal
                                      ? 'font-black text-[#00689D]'
                              ...
                              <input
                                ...
                                className="w-1/5 text-center border rounded p-1 text-xs [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none disabled:bg-gray-100 disabled:text-gray-500"
                              />
                    ...
                    <div className="space-y-4">
                        ...
                        (sec) => {
                          ...
                          return (
                            <div
                              ...
                              className="flex gap-2 items-center p-2 rounded-lg border bg-gray-50 border-gray-100"
                            >
                      ...
                      <div className="mt-6">
                        ...
                        <input
                          ...
                          name="rango_edad"
                          value={
                            actividadEnEdicion.rango_edad ||
                            ''
                          }
                          onChange={
                            handleChangeSimple
                          }
                          placeholder="Ej: 15 a 18 años"
                          className="w-full border border-gray-300 rounded-lg p-2 text-sm disabled:bg-gray-100 disabled:text-gray-500 outline-none focus:border-[#00689D] mt-2"
                        />
                ...
                <div className="space-y-4">
                  ...
                  <textarea
                    ...
                    rows={3}
                    value={
                      actividadEnEdicion.descripcion ||
                      ''
                    }
                    onChange={
                      handleChangeSimple
                    }
                    className="w-full border border-gray-300 rounded-lg p-3 text-sm focus:border-[#00689D] outline-none resize-none disabled:bg-gray-100 disabled:text-gray-500"
                    placeholder="Describe brevemente lo que hicieron..."
                  />
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    {actividadEnEdicion.evidencias?.map(
                      (ev) => (
                        <div
                          key={
                            ev.id
                          }
                          className="relative aspect-video rounded-xl border overflow-hidden bg-gray-100 group"
                        >
                  ...
                  {(
                    ...
                    <p className="text-xs text-orange-500 font-bold mt-1">
                      Recuerda subir al menos 1 fotografía
                      antes de enviar tu reporte final.
                    </p>
                  )}
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50 flex gap-3 shrink-0 rounded-b-2xl">
              ...
              <button
                ...
                className="flex-1 py-3 rounded-xl font-bold text-gray-600 bg-white border border-gray-300 hover:bg-gray-100 transition-colors"
              >
                Cancelar
              </button>
              ...
              <button
                ...
                className="flex-1 py-3 rounded-xl font-bold text-white bg-[#00689D] hover:bg-[#00527A] disabled:bg-gray-400 flex items-center justify-center gap-2 shadow-md transition-colors"
              >
      ...
      <div className="mb-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center shrink-0">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Completa y reporta las actividades de cada mes
          </p>
                  ...
                  <div
                    ...
                    className={`
                      ...
                      ${
                        reporteSeleccionado?.id ===
                        reporte.id
                          ? 'bg-blue-50 border-l-4 border-l-[#00689D]'
                          : 'hover:bg-gray-50'
                      }
                    `}
                  >
        ...
        {reporteSeleccionado ? (
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col min-w-0">
            ...
            <div className="p-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center shrink-0">
              ...
              <div>
                ...
                <h2 className="text-lg font-black text-gray-800">
                  Reporte del mes de{' '}
                  {
                    reporteSeleccionado.nombre_mes
                  }
                </h2>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <p className="text-xs text-gray-600">
                    Gestiona las actividades de este periodo
                  </p>

                  {reporteBloqueado && (
                    <span className="text-[10px] bg-gray-200 text-gray-600 px-2 py-1 rounded-full font-bold uppercase">
                      Edición bloqueada
                    </span>
                  )}
                ...
                className={`
                  ...
                  ${
                    ...
                    actividades.length ===
                      ...
                      : 'bg-[#00689D] text-white hover:bg-[#00527A] shadow-md'
                  }
                `}
              >
                <Send size={16} />
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
              ...
              {reporteBloqueado && (
                <div className="w-full mb-6 p-4 bg-gray-100 border border-gray-200 rounded-xl flex items-center gap-3 text-gray-600">
                  ...
                  <p className="text-sm">
                    Este reporte está{' '}
                    <strong>
                      {reporteSeleccionado.estado.toLowerCase()}
                    </strong>
              ...
              <div className="space-y-4">
                ...
                0 ? (
                  ...
                  <div className="text-center py-12 text-gray-500">
                    ...
                    {!reporteBloqueado && (
                      <p className="text-sm">
                        Agrega actividades para este mes
                        para poder enviar tu reporte
                      </p>
                    )}
                      ...
                      return (
                        <div
                          ...
                          className={`
                            ...
                            ${
                              puedeAbrir
                                ? 'hover:shadow-md cursor-pointer hover:border-[#00689D]/50 border-gray-200'
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <h4 className="font-bold text-gray-900 truncate">
                                ...
                                {!act.es_propia && (
                                  <span className="ml-2 text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full uppercase tracking-wider font-bold">
                                    Compartida
                                  </span>
                                )}

                                {act.es_externa && (
                                  <span className="ml-2 text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full uppercase tracking-wider font-bold">
                                    Externa
                                  </span>
                                )}
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 flex items-center justify-center">
            ...
            <div className="text-center text-gray-500">
              ...
              <p className="text-sm">
                Elige un reporte para gestionar sus actividades
              </p>
