...
//       // 3. Actividades Dinámicas (Incluyendo globales con municipio_id.is.null)
//       if (miMunicipioId) {
//         const hoyLocal = getTodayString();
...
//                       <h4 className="font-bold text-gray-900 text-sm md:text-base line-clamp-1 group-hover:text-[#00689D] transition-colors">{act.nombre}</h4>
//                       <p className="text-xs text-gray-500 font-medium mt-1">
//                         Por {act.es_externa && act.instituciones?.nombre ? act.instituciones.nombre : `${act.creador?.nombre || ''} ${act.creador?.apellido || ''}`} • {formatFechaAbreviada(act.fecha_evento).dia} {formatFechaAbreviada(act.fecha_evento).mes}
//                       </p>
//                     </div>
//                     <button className="w-8 h-8 rounded-full bg-gray-100 text-gray-400 flex items-center justify-center group-hover:bg-[#00689D] group-hover:text-white transition-colors shrink-0">
//                       <ChevronRight size={16} />
...
//                       {eventoSeleccionado.direccion && <p className="text-gray-600 italic"><span className="font-bold not-italic text-gray-700">Ref:</span> {eventoSeleccionado.direccion}</p>}
//                     </div>
//                     <p className="text-[#00689D] font-bold text-sm mt-2">{eventoSeleccionado.municipios?.nombre || 'General (Todos los municipios)'}</p>
//                   </div>
//                 </div>
//               </div>
...
//                       : `${eventoSeleccionado.creador?.nombre || ''} ${eventoSeleccionado.creador?.apellido || ''}`
//                     }
//                     {eventoSeleccionado.creado_por_usuario_id === usuarioDatos?.id && !eventoSeleccionado.es_externa && <span className="ml-2 text-[#00689D] font-black text-sm">(TÚ)</span>}
//                   </p>
//                 </div>
//               </div>
...
import { 
  Sun, Moon, Sunrise, Users, Calendar, Target, 
  MapPin, Clock, ArrowRight, ChevronRight, Activity, Globe, User,
  X, CheckCircle2, Edit2, Activity as ActivityIcon, Trash2, Ban, Building2
} from 'lucide-react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { format } from 'date-fns';
import { es } from 'date-fns/locale';
...
export default function EmbajadorInicio() {
    ...
    try {
      ...
      if (miMunicipioId) {
        ...
        if (upcomingActs) {
          ...
          upcomingActs.forEach((act) => {
            // AQUÍ LA MAGIA: Es de tu agenda personal SOLO si la creaste TÚ y NO es de una institución,
            // o si diste clic explícitamente en "Unirme a la actividad" como asistente.
            const esMiaPersonal = act.creado_por_usuario_id === usuarioDatos.id && !act.es_externa;
            const soyAsistente = act.actividad_asistentes?.some(a => a.usuario_id === usuarioDatos.id);

            if (esMiaPersonal || soyAsistente) {
              myActs.push(act);
            } else {
              communityActs.push(act);
            }
      ...
      console.error("Error cargando dashboard:", error);
      notifyWithSound(error?.message || 'No fue posible cargar tu panel.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSumarse = async () => {
    ...
    // Solo se bloquea si el evento es personal tuyo. Si es de una institución, sí te puedes unir.
    const esMiaPersonal = eventoSeleccionado.creado_por_usuario_id === usuarioDatos.id && !eventoSeleccionado.es_externa;
    if (esMiaPersonal) {
      notifyWithSound('No necesitas unirte a una actividad personal que tú organizaste.', 'info');
      return;
    }
    ...
    try {
      ...
      notifyWithSound(error?.message || 'No fue posible unirte a la actividad.', 'error');
    } finally {
      setIsJoining(false);
    }
  };
  ...
  // Comprobación para los botones de acción del Drawer
                ...
                {actividadesComunidad.map(act => (
                  <div key={act.id} onClick={() => abrirDetalle(act)} className="group relative flex items-center justify-between p-4 border border-gray-100 rounded-xl hover:bg-gray-50 hover:border-gray-200 hover:shadow-sm cursor-pointer transition-all">
                    <div className="pr-4">
                      ...
                      {/* Aquí se muestra la Institución o el Creador */}
                      <p className="text-xs text-gray-500 font-medium mt-1 flex items-center gap-1">
                        {act.es_externa && act.instituciones?.nombre ? (
                          <><Building2 size={12} className="text-purple-600"/> {act.instituciones.nombre}</>
                        ) : (
                          `Por ${act.creador?.nombre || ''} ${act.creador?.apellido || ''}`
                        )} 
                        <span>• {formatFechaAbreviada(act.fecha_evento).dia} {formatFechaAbreviada(act.fecha_evento).mes}</span>
                      </p>
        ...
        {eventoSeleccionado && (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8">
              ...
              {/* ORGANIZADOR DESTACADO AQUÍ */}
              <div className="flex items-start gap-4 pt-6 border-t border-gray-100">
                <div className="p-3 bg-blue-50 text-[#00689D] rounded-xl shrink-0">
              ...
              {eventoSeleccionado.estado !== 'Borrador' && (
                <div className="pt-6 border-t border-gray-100">
                  ...
                  {/* LÓGICA DEL BOTÓN: Si NO es un evento personal, se muestra el botón */}
                  {!esMiEventoPersonal && eventoSeleccionado.estado !== 'Cancelada' ? (
                    eventoSeleccionado.actividad_asistentes?.some(a => a.usuario_id === usuarioDatos?.id) ? (
                      <div className="flex flex-col gap-3 w-full">
                        <div className="w-full flex items-center justify-center gap-2 bg-green-50 text-green-700 border border-green-200 px-6 py-3 rounded-xl font-bold"><CheckCircle2 size={20} /> ¡Ya estás unido!</div>
                        <button onClick={handleDesunirse} disabled={isJoining} className="w-full flex items-center justify-center gap-2 bg-white text-red-500 border border-red-200 px-6 py-2.5 rounded-xl font-bold hover:bg-red-50 shadow-sm disabled:opacity-50">
