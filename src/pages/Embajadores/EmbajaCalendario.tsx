...
import { 
  ...
  PlusCircle, Target, Activity as ActivityIcon, Trash2, Ban
} from 'lucide-react';
...
export default function EmbajaCalendario() {
  ...
  // 1. Calcula dinámicamente el espacio del Header
  useEffect(() => {
    ...
    setTimeout(calcularEspacio, 50); // Retraso mínimo para asegurar que el DOM cargó

    return () => window.removeEventListener('scroll', calcularEspacio);
  }, []);

  // 2. Bloquea el scroll del fondo cuando el panel lateral está abierto
  useEffect(() => {
    if (isSidebarOpen) {
  ...
  // ==========================================
  // LÓGICA DE FECHAS (Corrección de Zona Horaria)
  // ==========================================
  ...
  // ==========================================
  // CARGA DE DATOS Y SINCRONIZACIÓN EN VIVO
  // ==========================================
          ...
          <div key={day.toString()} onClick={() => { setCurrentDate(cloneDay); setViewMode('dia'); }} className={`min-h-30 p-2 border-b border-r border-gray-100 cursor-pointer group ${!isSameMonth(day, monthStart) ? 'bg-gray-50/50 text-gray-400' : 'bg-white hover:bg-blue-50/30'}`}>
            ...
            <div className="mt-2 space-y-1">
              {acts.slice(0, 3).map(act => (
                <div key={act.id} onClick={(e) => { e.stopPropagation(); setEventoSeleccionado(act); setIsSidebarOpen(true); }} className={`px-2 py-1 rounded text-xs truncate font-medium shadow-sm ${act.estado === 'Borrador' ? 'bg-gray-100 text-gray-600 border border-gray-200' : 'bg-blue-50 text-[#00689D] border border-blue-100 hover:bg-[#00689D] hover:text-white transition-colors'}`}>
          ...
          return (
            <div key={i} className="flex flex-col min-h-125 border-r border-gray-100 bg-white">
              ...
              <div className="p-2 space-y-2 flex-1 bg-gray-50/30">
                {acts.map(act => (
                  <div key={act.id} onClick={() => { setEventoSeleccionado(act); setIsSidebarOpen(true); }} className={`border-l-4 p-2 rounded shadow-sm cursor-pointer hover:shadow-md transition-all group ${act.estado === 'Borrador' ? 'bg-white border-gray-400' : 'bg-white border-[#00689D]'}`}>
  ...
  const renderDayView = () => {
    ...
    if (acts.length === 0) return <div className="py-24 text-center"><CalendarIcon className="mx-auto h-16 w-16 text-gray-200 mb-4" /><h3 className="text-xl font-bold text-gray-800">Día libre</h3><p className="text-gray-500">No hay actividades programadas.</p></div>;
    return (
      <div className="p-6 max-w-4xl mx-auto space-y-4">
        {acts.map(act => (
          <div key={act.id} onClick={() => { setEventoSeleccionado(act); setIsSidebarOpen(true); }} className="flex flex-col sm:flex-row gap-4 p-5 rounded-2xl border bg-white cursor-pointer hover:shadow-md transition-all group">
            <div className={`flex flex-col items-center justify-center min-w-25 h-full py-2 rounded-xl border ${act.estado === 'Borrador' ? 'bg-gray-100 text-gray-500 border-gray-200' : 'bg-blue-50 text-[#00689D] border-blue-100'}`}>
            ...
            <div className="flex-1">
              <h3 className={`text-xl font-bold transition-colors ${act.estado === 'Borrador' ? 'text-gray-600' : 'text-gray-900 group-hover:text-[#00689D]'}`}>{act.nombre}</h3>
            ...
            <div className="flex items-center justify-end">
              <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border ${
                  act.estado === 'Borrador' ? 'bg-gray-100 text-gray-600 border-gray-300' :
