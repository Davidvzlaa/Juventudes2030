...
//   Info, X, LayoutGrid, List as ListIcon, Columns, User, Users, CheckCircle2, Edit2, 
...
//     isOpen: boolean;
//     tipo: 'eliminar' | 'cancelar_evento' | 'desunir' | 'descartar_form' | null;
...
//     setLoading(true);
//     try {
//       // 1. Obtener Municipio del Embajador
...
//   useEffect(() => { fetchDatosBase(); }, [usuarioDatos, location.key]);
...
//     const esParticipante = esCreador || act.actividad_asistentes?.some(a => a.usuario_id === usuarioDatos?.id);

//     if (act.estado === 'Cancelada') {
//       return {
//         mes: 'bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 hover:text-red-900',
...
//       } 
//       else if (dialogoConfirmacion.tipo === 'desunir') {
//         const { error } = await supabase.from('actividad_asistentes').delete().match({ actividad_id: dialogoConfirmacion.idActividad, usuario_id: usuarioDatos?.id });
//         if (error) throw error;
//         notifyWithSound('Te has desunido de la actividad exitosamente.', 'success');
//       } 
...
//         <p className="text-gray-500">No hay actividades programadas para {isPast ? 'este día' : 'hoy'} en tu municipio.</p>
//       </div>
//     );
...
//     // Filtramos para que solo vea las que él creó o a las que asiste
//     const misActividades = actividades.filter(a => a.creado_por_usuario_id === usuarioDatos?.id || a.actividad_asistentes.some(as => as.usuario_id === usuarioDatos?.id));
//     const todayStr = format(new Date(), 'yyyy-MM-dd');

//     const filtradas = misActividades.filter(a => {
...
//             <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2"><ListIcon className="text-[#00689D]" size={20} /> Mis Registros</h3>
...
//                 <p className="text-gray-400 text-sm">Cambia los filtros o añade una nueva actividad.</p>
//               </div>
//             ) : (
...
//                         <td className="p-4 font-bold text-gray-900">{a.nombre}</td>
//                         <td className="p-4 text-center">
//                           <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider inline-block ${esCrea ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'}`}>{esCrea ? 'Creador' : 'Participante'}</span>
//                         </td>
//                         <td className="p-4">
...
//             <CalendarIcon className="text-[#00689D]" size={36} /> Mi Agenda
//           </h1>
//           <p className="text-gray-500 mt-2 text-base max-w-xl">
//             Gestiona tu impacto, organiza tus próximos eventos y descubre cómo sumarte a las actividades de otros embajadores en tu municipio.
//           </p>
//         </div>
...
//             <PlusCircle size={20} /> Añadir Actividad
//           </button>
//         </div>
//       </div>
...
//             <span className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-slate-300"></div> Cartelera Pública</span>
...
//                     </div>
//                     <p className={`${eventoSeleccionado.estado === 'Cancelada' ? 'text-red-600' : 'text-[#00689D]'} font-bold text-sm mt-2`}>{eventoSeleccionado.municipios?.nombre || 'Municipio no definido'}</p>
//                   </div>
//                 </div>
//               </div>
...
//                           {asistente.usuario_id === usuarioDatos?.id && <span className={`ml-auto text-xs px-2 py-1 rounded-md font-bold ${eventoSeleccionado.estado === 'Cancelada' ? 'text-red-600 bg-red-50' : 'text-[#00689D] bg-blue-50'}`}>Tú</span>}
//                         </div>
//                       ))}
//                     </div>
...
//                         <button onClick={() => setDialogoConfirmacion({ isOpen: true, tipo: 'desunir', idActividad: eventoSeleccionado.id })} className="w-full flex items-center justify-center gap-2 bg-white text-red-500 border border-red-200 px-6 py-2.5 rounded-xl font-bold hover:bg-red-50 shadow-sm transition-colors">
//                           <X size={18} /> Desunirme de la actividad
//                         </button>
//                       </div>
//                     ) : (
...
//                   <FileText className="text-[#00689D]" size={28} /> {editingId ? 'Editar Actividad' : 'Nueva Actividad'}
//                 </h2>
//                 <p className="text-gray-500 mt-1 text-sm">Completa la información para registrarla en tu agenda.</p>
//               </div>
...
//               <form id="form-actividad" onSubmit={handleSubmitForm} className="space-y-6">
...
//                       <input type="text" required className="w-full p-2.5 border border-gray-300 rounded-lg outline-none focus:border-[#00689D] text-sm" value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} placeholder="Ej. Taller de Reforestación Juvenil" />
//                     </div>
//                     <div className="md:col-span-2">
...
//                     </div>
//                     <div className="md:col-span-2">
//                       <label className="block text-xs font-bold text-gray-700 mb-1">Descripción de la Dinámica <span className="text-red-500">*</span></label>
//                       <textarea required rows={3} className="w-full p-2.5 border border-gray-300 rounded-lg resize-none outline-none focus:border-[#00689D] text-sm" value={formData.descripcion} onChange={(e) => setFormData({...formData, descripcion: e.target.value})} placeholder="Explica el objetivo, dinámicas o requerimientos..."></textarea>
//                     </div>
//                   </div>
//                 </div>
...
//                     </div>
//                     <div>
//                       <label className="block text-xs font-bold text-gray-700 mb-1">Lugar Exacto (Edificio / Espacio) <span className="text-red-500">*</span></label>
//                       <input type="text" required className="w-full p-2.5 border border-gray-300 rounded-lg outline-none focus:border-[#00689D] text-sm" value={formData.lugar} onChange={(e) => setFormData({...formData, lugar: e.target.value})} placeholder="Ej. Plazuela 27 de Septiembre" />
//                     </div>
//                   </div>
...
//                       <input type="text" required className="w-full p-2.5 border border-gray-300 rounded-lg outline-none focus:border-[#00689D] text-sm" value={formData.colonia} onChange={(e) => setFormData({...formData, colonia: e.target.value})} placeholder="Colonia o Sector" />
//                       <input type="text" className="w-full p-2.5 border border-gray-300 rounded-lg outline-none focus:border-[#00689D] text-sm" value={formData.direccion} onChange={(e) => setFormData({...formData, direccion: e.target.value})} placeholder="Referencias (Ej. Frente al Oxxo)" />
//                     </div>
//                   </div>
//                 </div>
//               </form>
...
//                 <button form="form-actividad" type="submit" name="accionBoton" value="Borrador" disabled={isSaving} className="w-full sm:w-auto px-6 py-2.5 font-bold text-[#00689D] bg-blue-50 border border-[#00689D] rounded-xl hover:bg-blue-100 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
//                   <Archive size={16} /> Guardar Borrador
//                 </button>
//               )}

//               <button form="form-actividad" type="submit" name="accionBoton" value="Programada" disabled={isSaving} className="w-full sm:w-auto px-8 py-2.5 font-bold text-white bg-[#00689D] rounded-xl hover:bg-[#00527A] shadow-md transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
...
//                  dialogoConfirmacion.tipo === 'cancelar_evento' ? 'Cancelar Actividad' : 
//                  dialogoConfirmacion.tipo === 'desunir' ? 'Desunirme de la Actividad' :
//                  '¿Descartar Cambios?'}
//               </h3>
//               <p className="text-sm text-gray-600 mb-6">
//                 {dialogoConfirmacion.tipo === 'eliminar' ? '¿Estás seguro de que deseas eliminar permanentemente esta actividad? Esta acción no se puede deshacer.' : 
//                  dialogoConfirmacion.tipo === 'cancelar_evento' ? '¿Estás seguro de que deseas marcar esta actividad como Cancelada? Los asistentes verán este cambio.' : 
//                  dialogoConfirmacion.tipo === 'desunir' ? '¿Estás seguro de que deseas desunirte de esta actividad? Dejarás de aparecer en la lista de embajadores.' :
...
//                 <button onClick={() => !isProcessingAction && setDialogoConfirmacion({ isOpen: false, tipo: null, idActividad: null })} disabled={isProcessingAction} className="flex-1 px-4 py-2.5 bg-white border border-gray-300 rounded-xl text-sm font-bold text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50">
//                   {dialogoConfirmacion.tipo === 'descartar_form' ? 'Seguir editando' : 'Regresar'}
//                 </button>
//                 <button onClick={ejecutarAccionConfirmada} disabled={isProcessingAction} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-red-600 rounded-xl text-sm font-bold text-white hover:bg-red-700 transition-colors disabled:opacity-70">
//                   {isProcessingAction && <Loader2 size={16} className="animate-spin" />}
//                   {dialogoConfirmacion.tipo === 'eliminar' ? 'Sí, eliminar' : dialogoConfirmacion.tipo === 'cancelar_evento' ? 'Sí, cancelar' : dialogoConfirmacion.tipo === 'desunir' ? 'Sí, desunirme' : 'Sí, descartar'}
//                 </button>
//               </div>
//             </div>
//           </div>
...
import { 
  ...
  Info, X, LayoutGrid, List as ListIcon, Columns, User, Users, CheckCircle2, Edit2, 
  PlusCircle, Target, Activity as ActivityIcon, Trash2, Ban, AlertCircle, Loader2,
  FileText, Filter, Archive, Save, Globe, Building2
} from 'lucide-react';
import { toast } from 'sonner';
import { supabase } from '../../lib/supabase';
import { useAuth } from '../../hooks/useAuth';
...
export default function EmbajadorAgenda() {
  ...
  const obtenerEstilosUI = (act: Actividad) => {
    const esMiEventoPersonal = act.creado_por_usuario_id === usuarioDatos?.id && !act.es_externa;
    const soyAsistente = act.actividad_asistentes?.some(a => a.usuario_id === usuarioDatos?.id);
    const esParticipante = esMiEventoPersonal || soyAsistente;
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await guardarActividadCalendario({
        ...
        tipoAccionId: formData.tipo_accion_id,
        odsIds: odsSeleccionados,
        es_externa: false,      // Forzamos a que sea actividad personal
        institucion_id: null    // Forzamos nulo
      });
  ...
  const handleSumarse = async () => {
    ...
    const esMiEventoPersonal = eventoSeleccionado.creado_por_usuario_id === usuarioDatos.id && !eventoSeleccionado.es_externa;
    if (esMiEventoPersonal) {
      notifyWithSound('No necesitas unirte a una actividad personal que tú organizaste.', 'warning');
      return;
    }
  ...
  // ==========================================
  const renderListView = () => {
    // Filtramos para que solo vea las que él creó o a las que asiste (aunque sean externas)
    const misActividades = actividades.filter(a => a.creado_por_usuario_id === usuarioDatos?.id || a.actividad_asistentes.some(as => as.usuario_id === usuarioDatos?.id));
    const todayStr = format(new Date(), 'yyyy-MM-dd');
          ...
          <div className="overflow-x-auto">
            {filtradas.length === 0 ? (
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtradas.map((a) => {
                    const esCreadorPersonal = a.creado_por_usuario_id === usuarioDatos?.id && !a.es_externa;
                    const esGestorExterno = a.creado_por_usuario_id === usuarioDatos?.id && a.es_externa;
                    
                    let rolTexto = "Participante";
                    let rolEstilo = "bg-gray-100 text-gray-600";

                    if (esCreadorPersonal) {
                      rolTexto = "Creador";
                      rolEstilo = "bg-blue-100 text-blue-700";
                    } else if (esGestorExterno) {
                      rolTexto = "Gestor (Inst.)";
                      rolEstilo = "bg-purple-100 text-purple-700";
                    }
                    ...
                    return (
                      <tr key={a.id} className="hover:bg-blue-50/50 transition-colors">
                        <td className="p-4 font-bold text-gray-900">{a.nombre}</td>
                        <td className="p-4 text-center">
                          <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider inline-block ${rolEstilo}`}>
                            {rolTexto}
                          </span>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4 flex gap-2 justify-center items-center h-full">
                          <button type="button" onClick={() => abrirDetalles(a)} className="p-2 text-gray-600 hover:bg-gray-200 rounded-lg transition-colors" title="Ver Detalles"><Info size={18} /></button>
                          {(esCreadorPersonal || esGestorExterno) && (
                            <>
                              <button type="button" onClick={() => prepararEdicion(a)} className="p-2 text-blue-600 hover:bg-blue-100 rounded-lg transition-colors" title="Editar"><Edit2 size={18} /></button>
                              <button type="button" onClick={() => setDialogoConfirmacion({ isOpen: true, tipo: 'eliminar', idActividad: a.id })} className="p-2 text-red-600 hover:bg-red-100 rounded-lg transition-colors" title="Eliminar"><Trash2 size={18} /></button>
                            </>
        ...
        {drawerState.mode === 'detalles' && eventoSeleccionado && (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8">
              ...
              {/* ===== ORGANIZADOR ===== */}
              <div className="flex items-start gap-4 pt-6 border-t border-gray-100">
                <div className={`p-3 rounded-xl shrink-0 ${eventoSeleccionado.estado === 'Cancelada' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-[#00689D]'}`}>
                  {eventoSeleccionado.es_externa ? <Building2 size={24} /> : <User size={24} />}
                </div>
                <div>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-1">Organizador</p>
                  <p className="font-bold text-gray-900 text-lg">
                    ...
                    {eventoSeleccionado.creado_por_usuario_id === usuarioDatos?.id && !eventoSeleccionado.es_externa && (
                      <span className={`ml-2 font-black text-sm ${eventoSeleccionado.estado === 'Cancelada' ? 'text-red-600' : 'text-[#00689D]'}`}>(TÚ)</span>
                    )}
                  </p>
                </div>
              </div>
              ...
              {eventoSeleccionado.estado !== 'Borrador' && (
                <div className="pt-6 border-t border-gray-100">
                  ...
                  {/* BOTÓN PARA UNIRSE / DESUNIRSE */}
                  {!(eventoSeleccionado.creado_por_usuario_id === usuarioDatos?.id && !eventoSeleccionado.es_externa) && eventoSeleccionado.estado !== 'Cancelada' ? (
                    eventoSeleccionado.actividad_asistentes?.some(a => a.usuario_id === usuarioDatos?.id) ? (
                      <div className="flex flex-col gap-3 w-full">
                        <div className="w-full flex items-center justify-center gap-2 bg-green-50 text-green-700 border border-green-200 px-6 py-3 rounded-xl font-bold"><CheckCircle2 size={20} /> ¡Ya estás unido!</div>
                        <button onClick={() => setDialogoConfirmacion({ isOpen: true, tipo: 'desunir', idActividad: eventoSeleccionado.id })} className="w-full flex items-center justify-center gap-2 bg-white text-red-500 border border-red-200 px-6 py-2.5 rounded-xl font-bold hover:bg-red-50 shadow-sm transition-colors">
